...
Column {
    ...
    Repeater {
        ...
        delegate: ServiceItemBase {
            property var appInfo: ApplicationManager.applicationInfo(model.applicationId, "unconfined")
            text: appInfo.displayName ? appInfo.displayName : model.applicationId
            subText: appInfo.displayId
            iconSource: appInfo.icon.indexOf("/") === 0 ?
                appInfo.icon : "image://theme/" + appInfo.icon
